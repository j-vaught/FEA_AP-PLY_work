...
RESULTS_DIR = STAGE_DIR / "results_richviz"
FIGURES_DIR = STAGE_DIR / "figures"
RUN_LOG = STAGE_DIR / "run_richviz.log"
RICHVIZ_FRAME_COUNT = 60
...
def wait_for_solver_slot(log: Logger) -> None:
    while True:
        proc = subprocess.run(
            ["pgrep", "-a", "engine_linux64"],
            text=True,
            stdout=subprocess.PIPE,
        ...
        running = [
            ...
            if "engine_linux64" in line and "stage01_" not in line
        ]
        if not running:
            return
        log.log("[richviz] another OpenRadioss engine is active; sleeping 60 s")
        time.sleep(60.0)
...
def write_engine(job: str, out_rad: pathlib.Path, run_time: float) -> None:
    lines = [
        ...
        "/ANIM/DT",
        "#            TSTART             dT_ANIM",
        fmt_f(0.0, run_time / RICHVIZ_FRAME_COUNT),
        "/ANIM/BRICK/TENS/STRESS/ALL",
        "/ANIM/BRICK/TENS/STRAIN/ALL",
        "/ANIM/BRICK/TENS/EPSP",
...
def convert_single_anim_to_vtk(anim_plain: pathlib.Path, log: Logger, env: dict[str, str]) -> pathlib.Path:
    log.log("$ " + " ".join([str(ANIM_TO_VTK), str(anim_plain)]))
    proc = subprocess.run(
        [str(ANIM_TO_VTK), str(anim_plain)],
        cwd=str(anim_plain.parent),
        env=env,
        text=True,
        stdout=subprocess.PIPE,
    ...
    final_vtk = anim_plain.with_suffix(anim_plain.suffix + ".vtk")
    if proc.stdout.lstrip().startswith("# vtk"):
        ...
        return final_vtk
    candidates = sorted(anim_plain.parent.glob(f"{anim_plain.name}*.vtk")) + sorted(anim_plain.parent.glob("*.vtk"))
    if not candidates:
        raise FileNotFoundError(f"anim_to_vtk produced no .vtk in {anim_plain.parent}")
    vtk = candidates[0]
    if vtk != final_vtk:
...
def convert_anim_series_to_vtk(job: str, workdir: pathlib.Path, log: Logger, env: dict[str, str]) -> list[pathlib.Path]:
    gz_frames = sorted(workdir.glob(f"{job}A[0-9][0-9][0-9].gz"))
    plain_frames = sorted(path for path in workdir.glob(f"{job}A[0-9][0-9][0-9]") if path.is_file())
    frame_names = sorted({path.name.removesuffix(".gz") for path in [*gz_frames, *plain_frames]})
    if not frame_names:
        raise FileNotFoundError(f"OpenRadioss animation frames not found for {job} in {workdir}")

    vtk_paths: list[pathlib.Path] = []
    for frame_name in frame_names:
        anim_plain = workdir / frame_name
        anim_gz = workdir / f"{frame_name}.gz"
        if anim_gz.exists() and (not anim_plain.exists() or anim_gz.stat().st_mtime > anim_plain.stat().st_mtime):
        ...
        vtk_paths.append(convert_single_anim_to_vtk(anim_plain, log, env))
    log.log(f"[{job}] converted {len(vtk_paths)} animation frames to VTK")
    return vtk_paths
...
def write_typst_figure(rows: list[dict[str, object]]) -> pathlib.Path:
    out = FIGURES_DIR / "stage01_richviz_convergence.typ"
...
def run_case(
    ...
    job = f"stage01_{load_case}_{mesh_label}_richviz"
    ...
    vtk_series = convert_anim_series_to_vtk(job, workdir, log, env)
    vtk = vtk_series[-1]
