...
def wait_for_solver_slot(log: Logger) -> None:
    while True:
        ...
        running = [
            ...
            if "engine_linux64" in line and "stage02_" not in line
        ]
        if not running:
            return
...
def convert_single_anim_to_vtk(anim: pathlib.Path, log: Logger, env: dict[str, str]) -> pathlib.Path:
...
def convert_anim_series_to_vtk(job: str, workdir: pathlib.Path, log: Logger, env: dict[str, str]) -> list[pathlib.Path]:
    ...
    if not frame_names:
        raise FileNotFoundError(f"animation frames not found for {job} in {workdir}")
    vtk_paths: list[pathlib.Path] = []
    for frame_name in frame_names:
        anim = workdir / frame_name
        gz = workdir / f"{frame_name}.gz"
        ...
        vtk_paths.append(convert_single_anim_to_vtk(anim, log, env))
...
def run_case(
    ...
    job = f"stage02_{solver_mode}_{mesh_label}_a{alpha_label}_richviz"
...
def write_typst_figure(rows: list[SweepRow]) -> pathlib.Path:
    out = FIGURES_DIR / "stage02_richviz_elastica.typ"
